#!/bin/bash
# check_versions.sh - Check for new commits in pecl-networking-ssh2
...
get_latest_commit() {
    local api_url="https://api.github.com/repos/php/pecl-networking-ssh2/commits/master"
    local commit_info=""
    local auth=()
...
# Function to get version from php_ssh2.h
get_version() {
    # Clone temporarily to get version
    local tmpdir=$(mktemp -d)
    local version=""
    if git clone --depth=1 https://github.com/php/pecl-networking-ssh2.git "$tmpdir/pecl-networking-ssh2" >/dev/null 2>&1; then
        version=$(sed -n '/#define PHP_SSH2_VERSION/{s/.*"\([^"]*\)".*/\1/p}' "$tmpdir/pecl-networking-ssh2/php_ssh2.h" 2>/dev/null)
        if [ -z "$version" ]; then
            version=$(cd "$tmpdir/pecl-networking-ssh2" && git describe --tags --abbrev=0 2>/dev/null | sed 's/^v\?RELEASE_//;s/_/./g')
        fi
    fi
    rm -rf "$tmpdir"

    # Hard-fail when the version cannot be determined. A hardcoded fallback
    # here would commit a fabricated version.
    if [ -z "$version" ]; then
        echo "ERROR: could not determine pecl-networking-ssh2 version from upstream" >&2
        return 1
    fi
...
read_current_versions() {
    if [ -f "$VERSIONS_FILE" ]; then
        CURRENT_COMMIT=$(grep -A3 '"php-ssh2"' "$VERSIONS_FILE" | grep '"commit"' | sed 's/.*"commit": *"//;s/".*//')
        CURRENT_VERSION=$(grep -A1 '"php-ssh2"' "$VERSIONS_FILE" | grep '"version"' | sed 's/.*"version": *"//;s/".*//')
    else
        CURRENT_COMMIT=""
        CURRENT_VERSION=""
    fi
    ...
    cat > "$VERSIONS_FILE" <<EOF
{
  "php-ssh2": {
    ...
    "pecl_url": "https://pecl.php.net/package/ssh2",
...
# Main execution
echo "=== PHP-SSH2 Version Check Script ==="
echo "Mode: $MODE"
...
# survive; a $() subshell call loses the date and message.
echo "Checking latest pecl-networking-ssh2 commit..."
