...
get_version() {
    ...
    cd "$tmpdir"
    git clone --depth=1 https://github.com/php/pecl-networking-ssh2.git >/dev/null 2>&1
    cd pecl-networking-ssh2
    VERSION=$(sed -n '/#define PHP_SSH2_VERSION/{s/.*"\([^"]*\)".*/\1/p}' php_ssh2.h)
    if [ -z "$VERSION" ]; then
        VERSION=$(git describe --tags --abbrev=0 2>/dev/null | sed 's/^v\?RELEASE_//;s/_/./g' || echo "1.4.1")
    fi
    cd "$SCRIPT_DIR"
    rm -rf "$tmpdir"
    echo "$VERSION"
